...
def test_panel(artifacts_path="artifacts"):
    ...
    try:
        ...
        print(f"✅ Panel sanity OK: {len(df)} rows, period range: {df['period'].min()} to {df['period'].max()}")
        return True
...
def test_metrics(artifacts_path="artifacts"):
    ...
    try:
        ...
        assert ({"metric", "value"} <= cols) or ({"metric_name", "metric_value"} <= cols), "metric columns missing"
        assert (df["model"] == "seasonal_naive").any(), "baseline missing"
        
        print(f"✅ Metrics sanity OK: {len(df)} rows, has baseline: {(df['model'] == 'seasonal_naive').any()}")
        return True
    except Exception as e:
...
def test_forecast(artifacts_path="artifacts"):
    ...
    try:
        ...
        print(f"✅ Forecast sanity OK: {len(df)} rows, future months: {(df['period'] > pd.Timestamp.now()).all()}")
        return True
    except Exception as e:
