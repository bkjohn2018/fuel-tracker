...
class EIAClient:
    ...
    def _get(self, url: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.debug("Making EIA API request", extra={"url": url, "params": params_with_key})
        ...
        if response.status_code >= 500:
            logger.warning("EIA API server error", extra={"status_code": response.status_code})
            raise requests.exceptions.HTTPError(f"Server error: {response.status_code}", response=response)
        
        response.raise_for_status()
        
        data = response.json()
        logger.debug("EIA API response received", extra={"status_code": response.status_code})
    ...
    def fetch_series(self, endpoint: str, params: Dict[str, Any]) -> pd.DataFrame:
        ...
        if not endpoint.startswith('http'):
            ...
            if endpoint_config:
                ...
                logger.info("Using endpoint config", extra={"endpoint_key": endpoint, "url": url})
            else:
                # Fallback to treating as relative endpoint
                url = f"{self.BASE_URL}/{endpoint.lstrip('/')}"
                merged_params = params
                logger.info("Using relative endpoint", extra={"endpoint": endpoint, "url": url})
        else:
            url = endpoint
            merged_params = params
        ...
        try:
            logger.info("Fetching EIA series", extra={"endpoint": endpoint, "url": url, "params": merged_params})
            data = self._get(url, merged_params)
            
            # Normalize JSON response to DataFrame
            df = self._normalize_response(data)
            
            logger.info("Series fetched successfully", extra={
                "endpoint": endpoint, 
                "url": url,
                "rows": len(df),
            ...
            logger.error("Failed to fetch EIA series", extra={
                "endpoint": endpoint,
                "url": url,
                "error": str(e)
            })
    ...
    def _normalize_response(self, data: Dict[str, Any]) -> pd.DataFrame:
        ...
        try:
            ...
            raise ValueError(f"Failed to normalize EIA response: {e}")
