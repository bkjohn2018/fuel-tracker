...
def read_panel(path: Path) -> pd.DataFrame:
    ...
    if not path.exists():
        logger.info(
            "Panel file does not exist, returning empty DataFrame",
            extra={"path": str(path)},
        )
...
def append_revision(df: pd.DataFrame, path: Path) -> None:
    ...
    if path.exists():
        ...
        combined_df = combined_df.drop_duplicates(
            subset=['period', 'batch_id'], keep='last'
        )
        ...
        logger.info(
            "Appending revision to existing panel",
            extra={
                "path": str(path),
...
def write_lineage_log(
    ...
    try:
        ...
        logger.error(
            ...
            extra={"error": str(e), "log_path": str(log_path)},
        )
        raise
