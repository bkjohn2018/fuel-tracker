...
class SARIMAX:
    ...
    def __init__(self, 
        ...
        logger.info("SARIMAX model initialized", extra={
            "order": order,
    ...
    def fit(self, y: pd.Series, exog: Optional[pd.DataFrame] = None) -> 'SARIMAX':
        ...
        if len(y) < self.seasonal_order[3] * 2:
            raise ValueError(f"Training data must have at least {self.seasonal_order[3] * 2} observations")
        ...
        if exog is not None and not exog.empty and self.exog_columns:
            ...
            logger.info("Exogenous variables fitted", extra={"exog_coefs": self.exog_coefs})
        else:
        ...
        logger.info("SARIMAX model fitted", extra={
    ...
    def predict(self, horizon: int, exog: Optional[pd.DataFrame] = None) -> np.ndarray:
        ...
        for i in range(horizon):
            ...
            forecast = current_values[-1] + seasonal_component + trend_component + exog_component
        ...
        logger.debug("Generated SARIMAX forecasts", extra={
            "horizon": horizon,
            "seasonal_period": self.seasonal_order[3],
            "has_exog": self.exog_coefs is not None
        })
    ...
    def fit_predict(self, y: pd.Series, horizon: int, exog: Optional[pd.DataFrame] = None) -> np.ndarray:
        """
    ...
    def _fit_exog_relationship(self, y: pd.Series, exog: pd.DataFrame) -> Dict[str, float]:
        """
        ...
        for col in self.exog_columns:
            if col in exog_aligned.columns:
                ...
                correlation = np.corrcoef(y_aligned.values, exog_aligned[col].values)[0, 1]
                if not np.isnan(correlation):
                    # Scale coefficient by target std
                    coefs[col] = correlation * np.std(y_aligned) / np.std(exog_aligned[col])
                else:
                    coefs[col] = 0.0
            else:
                coefs[col] = 0.0
...
def rolling_backtest(
    ...
    **model_kwargs
    ...
    logger.info("Starting SARIMAX rolling backtest", extra={
        "total_length": len(y),
    ...
    for i in range(horizon, len(y_backtest)):
        ...
        actual_exog = exog_backtest.iloc[i:i+horizon] if exog_backtest is not None else None
    ...
    logger.info("SARIMAX rolling backtest completed", extra={
...
def _calculate_split_metrics(actual: np.ndarray, forecast: np.ndarray) -> Dict[str, float]:
    """
