...
class STLETS:
    """STL-ETS forecasting model combining seasonal decomposition with exponential smoothing."""
    
    def __init__(self, period: int = 12, seasonal_window: int = 7, trend_window: int = 21):
        """
        ...
        logger.info("STL-ETS model initialized", extra={
            "period": period,
    ...
    def fit(self, y: pd.Series) -> 'STLETS':
        ...
        if len(y) < self.period * 2:
            raise ValueError(f"Training data must have at least {self.period * 2} observations")
        ...
        logger.info("STL-ETS model fitted", extra={
    ...
    def predict(self, horizon: int) -> np.ndarray:
        ...
        logger.debug("Generated STL-ETS forecasts", extra={
            "horizon": horizon,
            "period": self.period
        })
...
def rolling_backtest(
    ...
    **model_kwargs
    ...
    logger.info("Starting STL-ETS rolling backtest", extra={
        "total_length": len(y),
    ...
    logger.info("STL-ETS rolling backtest completed", extra={
