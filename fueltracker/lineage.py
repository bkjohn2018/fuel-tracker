...
from .contracts import BatchMeta, BatchId
...
def start_batch(source: str = "EIA", notes: Optional[str] = None) -> BatchMeta:
    ...
    logger.info("Started new batch", extra={
...
def attach_lineage_columns(
    ...
    logger.info("Attached lineage columns to DataFrame", extra={
...
def validate_lineage_columns(
    ...
    if missing_cols:
        logger.error("Missing required lineage columns", extra={"missing_cols": missing_cols})
        return False
    ...
    logger.debug("Lineage columns validation passed", extra={
        "batch_id_col": batch_id_col,
        "asof_ts_col": asof_ts_col,
        "rows": len(df)
    })
