...
from .contracts import MonthlyFuelRow, BatchMeta
...
def build_monthly_panel(raw_df: pd.DataFrame, batch: BatchMeta) -> pd.DataFrame:
    ...
    logger.info("Building monthly panel", extra={
        "raw_rows": len(raw_df),
    ...
    try:
        ...
        logger.info("Monthly panel built successfully", extra={
        ...
        logger.error("Failed to build monthly panel", extra={
            "error": str(e),
            "batch_id": str(batch.batch_id)
        })
        raise
...
def _enforce_month_end_dates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_copy['period'] = pd.to_datetime(df_copy['period']).dt.to_period('M').dt.to_timestamp('M')
    
    # Convert to date objects for consistency
    df_copy['period'] = df_copy['period'].dt.date
    
    logger.debug("Enforced month-end dates", extra={
        "date_range": f"{df_copy['period'].min()} to {df_copy['period'].max()}"
    })
...
def _validate_panel(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info("Panel validation passed", extra={
