...
from ..io_parquet import read_panel, append_revision, write_lineage_log
...
def fetch_and_build_panel(dry_run: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info("Data fetched from EIA API", extra={
            "raw_rows": len(raw_df),
        ...
        if not dry_run:
            ...
            logger.info("Files written successfully", extra={
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fetch EIA data and build monthly panel")
    parser.add_argument(
        "--dry-run", 
        action="store_true", 
        help="Print counts but don't write files"
    )
