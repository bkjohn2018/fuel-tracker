...
from .cache import get_last_success_info, is_cache_fresh
...
def should_block_publish() -> bool:
    ...
    if not cache_fresh:
        logger.warning(
            "Cache is stale, blocking publish",
            extra={"cache_ttl_days": CACHE_TTL_BUSINESS_DAYS},
        )
        return True
...
def get_provisional_status() -> Dict[str, Any]:
    ...
    if cache_fresh and last_success_info:
        ...
        status = {
            ...
            "last_asof": last_success_info.get("last_success_time", "none")
            if last_success_info
            else "none",
            "cache_file": last_success_info.get("last_success_file", "none")
            if last_success_info
            else "none",
        }
