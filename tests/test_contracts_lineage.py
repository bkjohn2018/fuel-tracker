...
class TestBatchMeta:
    ...
    def test_valid_batch_meta(self):
        ...
        batch_meta = BatchMeta(
            batch_id=batch_id, asof_ts=asof_ts, source="EIA", notes="Test batch"
        )
    ...
    def test_batch_meta_without_notes(self):
        ...
        batch_meta = BatchMeta(batch_id=batch_id, asof_ts=asof_ts, source="EIA")
    ...
    def test_batch_meta_timezone_validation(self):
        ...
        with pytest.raises(ValueError, match="asof_ts must be timezone-aware"):
            BatchMeta(batch_id=batch_id, asof_ts=asof_ts, source="EIA")
    ...
    def test_batch_meta_source_validation(self):
        ...
        with pytest.raises(ValueError):
            BatchMeta(batch_id=batch_id, asof_ts=asof_ts, source="INVALID_SOURCE")
...
class TestLineageFunctions:
    ...
    def test_attach_lineage_columns(self):
        ...
        df = pd.DataFrame(
            {'period': [date(2024, 1, 31), date(2024, 2, 29)], 'value': [75.5, 78.2]}
        )
    ...
    def test_validate_lineage_columns_missing(self):
        ...
        df = pd.DataFrame({'period': [date(2024, 1, 31)], 'value': [75.5]})
